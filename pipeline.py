...
def main() -> None:
    ...
    if bool(args.loso_all):
        ...
        row_re = re.compile(
            r"^\s*(CSP\+LDA|FBCSP\+LDA|EEGNet)\s+([0-9.]+)\s+([0-9.]+)\s+([0-9.\-]+)\s+([0-9.\-]+)",
            re.MULTILINE,
        )
        metrics_by_model: dict[str, dict[str, list[float]]] = {
            "CSP+LDA": {"acc_original": [], "kappa_original": []},
            "FBCSP+LDA": {"acc_original": [], "kappa_original": []},
            "EEGNet": {"acc_original": [], "kappa_original": []},
        }
        ...
        for held_out_sid in loso_subjects_cli:
            ...
            rows = list(row_re.finditer(proc.stdout))
            for m in rows:
                model = str(m.group(1))
                acc_orig = float(m.group(2))
                kappa_orig = float(m.group(4))
                if model in metrics_by_model:
                    metrics_by_model[model]["acc_original"].append(acc_orig)
                    metrics_by_model[model]["kappa_original"].append(kappa_orig)
        ...
        print("\n" + "=" * 80)
        print("LOSO-ALL SUMMARY (mean±std across held-out subjects)")
        print("=" * 80)
        for model in ("CSP+LDA", "FBCSP+LDA", "EEGNet"):
            m_acc, s_acc = _mean_std(metrics_by_model[model]["acc_original"])
            m_k, s_k = _mean_std(metrics_by_model[model]["kappa_original"])
            print(
                f"{model:10s} | Acc Original: {m_acc:.4f} ± {s_acc:.4f} | "
                f"Kappa Original: {m_k:.4f} ± {s_k:.4f}"
            )
        print("=" * 80 + "\n")
        return
    ...
    def _generate_ddpm_paper_figures() -> None:
        ...
        _log_section("Figure mode: train DDPM for channels [Fp1, AF4]")
        ...
        # ------------------------------
        # Figure 2: forward/reverse snapshots at t={0,500,999} with temporal(0-0.5s)+PSD.
        # ------------------------------
        fig2_dir = out_dir / "figure2_forward_reverse"
        fig2_dir.mkdir(parents=True, exist_ok=True)
        sample_idx = 0
        t_grid_forward = [0, 500, 999]
        reverse_steps = [999, 500, 0]
        n_half = max(1, int(round(0.5 * sf)))
        t_half = t_sec[:n_half]
        ch_colors = {"Fp1": "#1f77b4", "AF4": "#d62728"}

        def _welch_psd_1d(x: np.ndarray, fs_hz: float) -> tuple[np.ndarray, np.ndarray]:
            from scipy.signal import welch

            f, pxx = welch(np.asarray(x, dtype=np.float64), fs=float(fs_hz), nperseg=min(256, int(x.shape[0])))
            return f, 10.0 * np.log10(np.maximum(pxx, 1e-12))

        for ch_name, ch_idx in zip(wanted, ch_idxs):
            ch_color = ch_colors.get(ch_name, "#4c72b0")
            ...
            # Save one image per t for forward: temporal (0-0.5s) + PSD.
            for tt in t_grid_forward:
                ab = float(alphas_cumprod_fig[int(tt)])
                x_forward = np.sqrt(max(ab, 1e-12)) * x0 + np.sqrt(max(1.0 - ab, 1e-12)) * eps
                fig_f_t, ax_f_t = plt.subplots(1, 1, figsize=(6.2, 3.6))
                ax_f_t.plot(t_half, x0[:n_half], color="black", lw=1.2, label="Original")
                ax_f_t.plot(t_half, x_forward[:n_half], color=ch_color, lw=1.2, alpha=0.95, label=f"Forward t={tt}")
                ax_f_t.set_title(f"{ch_name} Forward t={tt} (0–0.5s)")
                ax_f_t.set_xlabel("Time (s)")
                ax_f_t.set_ylabel("Amplitude (a.u.)")
                ax_f_t.grid(alpha=0.25, lw=0.4)
                ax_f_t.legend(loc="upper right", fontsize=8, frameon=False)
                fig_f_t.tight_layout()
                tt_name = 1000 if int(tt) == 999 else int(tt)
                fig_f_t.savefig(
                ...
                f0, p0 = _welch_psd_1d(x0, sf)
                f1, p1 = _welch_psd_1d(x_forward, sf)
                m = (f0 >= 4.0) & (f0 <= 45.0)
                fig_f_p, ax_f_p = plt.subplots(1, 1, figsize=(6.2, 3.6))
                ax_f_p.plot(f0[m], p0[m], color="black", lw=1.2, label="Original")
                ax_f_p.plot(f1[m], p1[m], color=ch_color, lw=1.2, alpha=0.95, label=f"Forward t={tt}")
                ax_f_p.set_title(f"{ch_name} Forward t={tt} PSD")
                ax_f_p.set_xlabel("Frequency (Hz)")
                ax_f_p.set_ylabel("Power (dB)")
                ax_f_p.grid(alpha=0.25, lw=0.4)
                ax_f_p.legend(loc="upper right", fontsize=8, frameon=False)
                fig_f_p.tight_layout()
                fig_f_p.savefig(
                    fig2_dir / f"figure2_{ch_name.lower()}_forward_t{tt_name}_psd.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close(fig_f_p)

            # Save one image per t for reverse: temporal (0-0.5s) + PSD.
            for tt in reverse_steps:
                x_rev = reverse_series.get(int(tt))
                if x_rev is None:
                    continue
                fig_r_t, ax_r_t = plt.subplots(1, 1, figsize=(6.2, 3.6))
                ax_r_t.plot(t_half, x0[:n_half], color="black", lw=1.2, label="Original")
                ax_r_t.plot(t_half, x_rev[:n_half], color=ch_color, lw=1.2, alpha=0.95, label=f"Reverse t={tt}")
                ax_r_t.set_title(f"{ch_name} Reverse t={tt} (0–0.5s)")
                ax_r_t.set_xlabel("Time (s)")
                ax_r_t.set_ylabel("Amplitude (a.u.)")
                ax_r_t.grid(alpha=0.25, lw=0.4)
                ax_r_t.legend(loc="upper right", fontsize=8, frameon=False)
                ...
                f0, p0 = _welch_psd_1d(x0, sf)
                f1, p1 = _welch_psd_1d(x_rev, sf)
                m = (f0 >= 4.0) & (f0 <= 45.0)
                fig_r_p, ax_r_p = plt.subplots(1, 1, figsize=(6.2, 3.6))
                ax_r_p.plot(f0[m], p0[m], color="black", lw=1.2, label="Original")
                ax_r_p.plot(f1[m], p1[m], color=ch_color, lw=1.2, alpha=0.95, label=f"Reverse t={tt}")
                ax_r_p.set_title(f"{ch_name} Reverse t={tt} PSD")
                ax_r_p.set_xlabel("Frequency (Hz)")
                ax_r_p.set_ylabel("Power (dB)")
                ax_r_p.grid(alpha=0.25, lw=0.4)
                ax_r_p.legend(loc="upper right", fontsize=8, frameon=False)
                fig_r_p.tight_layout()
                fig_r_p.savefig(
                    fig2_dir / f"figure2_{ch_name.lower()}_reverse_t{tt_name}_psd.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close(fig_r_p)
        ...
        # ------------------------------
        # Figure 3: Original vs Hybrid (temporal, PSD, wavelet) for Fp1 and AF4.
        # ------------------------------
        ...
        psd_real = ep_real_2ch.compute_psd(method="welch", fmin=4.0, fmax=45.0, n_fft=256)
        psd_hyb = ep_hyb_2ch.compute_psd(method="welch", fmin=4.0, fmax=45.0, n_fft=256)
        psd_r, f_psd = psd_real.get_data(return_freqs=True)  # (n_ep, n_ch, n_f)
        psd_h, _ = psd_hyb.get_data(return_freqs=True)

        # Wavelet TFR (notebook-style with MNE tfr_morlet) on 8-30 Hz.
        # For paper illustration, build a 4-second segment by concatenating consecutive 1-second epochs.
        freqs_tfr = np.arange(8.0, 31.0, 1.0, dtype=np.float64)
        ...
        idx4 = np.arange(seg_epochs, dtype=int)
        labels4 = np.asarray(labels_test[idx4], dtype=int)

        def _label_to_name(v: int) -> str:
            vv = int(v)
            if vv == 2:
                return "left"
            if vv == 3:
        ...
        sec_labels = [_label_to_name(v) for v in labels4.tolist()]
        ...
        x4_real = x4_real[None, :, :].astype(np.float32)  # (1, 2, T4)
        x4_hyb = x4_hyb[None, :, :].astype(np.float32)
        ep_real_4s = mne.EpochsArray(x4_real, info_2ch, tmin=0.0, verbose=False)
        ep_hyb_4s = mne.EpochsArray(x4_hyb, info_2ch, tmin=0.0, verbose=False)
        ...
        pwr_real_data = np.asarray(pwr_real.data, dtype=np.float64)  # (n_ch, n_freq, n_time)
        pwr_hyb_data = np.asarray(pwr_hyb.data, dtype=np.float64)
        ...
        for row_i, ch_name in enumerate(wanted):
            ...
            for cond_name, x_arr, color in [
                ...
                fig_t, ax_t = plt.subplots(1, 1, figsize=(10, 3.5))
                ...
                ax_t.grid(alpha=0.25, lw=0.4)
                fig_t.tight_layout()
                fig_t.savefig(fig3_dir / f"figure3_{ch_name.lower()}_temporal_{cond_name}.png", dpi=300, bbox_inches="tight")
                plt.close(fig_t)

            # PSD: one image per condition.
            m_psd_r = np.mean(psd_r[:, row_i, :], axis=0)
            m_psd_h = np.mean(psd_h[:, row_i, :], axis=0)
            for cond_name, pxx, color in [
                ("original", m_psd_r, "black"),
                ("hybrid", m_psd_h, "#4c72b0"),
            ]:
                fig_p, ax_p = plt.subplots(1, 1, figsize=(6.8, 4.0))
                ax_p.plot(f_psd, 10.0 * np.log10(np.maximum(pxx, 1e-12)), color=color, lw=1.2)
                ax_p.set_title(f"{ch_name} — PSD Welch ({cond_name})")
                ax_p.set_xlabel("Frequency (Hz)")
                ax_p.set_ylabel("Power (dB)")
                ax_p.grid(alpha=0.25, lw=0.4)
                fig_p.tight_layout()
                fig_p.savefig(fig3_dir / f"figure3_{ch_name.lower()}_psd_{cond_name}.png", dpi=300, bbox_inches="tight")
                plt.close(fig_p)

            # Wavelet heatmap: one image per condition (MNE tfr_morlet).
            for cond_name, pwr in [
                ("original", pwr_real_data[row_i]),
                ("hybrid", pwr_hyb_data[row_i]),
            ]:
                fig_w, ax_w = plt.subplots(1, 1, figsize=(7.5, 4.5))
                im = ax_w.imshow(
                    np.log10(np.maximum(pwr, 1e-12)),
                    aspect="auto",
                    origin="lower",
                    extent=[float(pwr_real.times[0]), float(pwr_real.times[-1]), float(freqs_tfr[0]), float(freqs_tfr[-1])],
                    cmap="viridis",
                )
                ax_w.set_title(f"{ch_name} — Wavelet ({cond_name})")
                ax_w.set_xlabel("Time (s)")
                ax_w.set_ylabel("Frequency (Hz)")
                # Separation guides every 0.5 s (dataset stride), dotted style.
                duration_s = float(x4_real.shape[2]) / float(sf)
                half_marks = np.arange(0.0, duration_s + 1e-9, 0.5, dtype=np.float64)
                for s_mark in half_marks:
                    ax_w.axvline(float(s_mark), color="white", lw=0.6, alpha=0.35, linestyle=":")
