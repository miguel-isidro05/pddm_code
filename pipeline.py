...
def _per_epoch_zscore_2d(x: np.ndarray, *, eps: float = 1e-6) -> np.ndarray:
    """
    Per-epoch z-score for batches shaped (B, T).
    Used to stabilize synthesized channels for covariance-sensitive classifiers.
    """
    x = np.asarray(x, dtype=np.float32)
    mu = x.mean(axis=1, keepdims=True)
    sd = x.std(axis=1, keepdims=True)
    sd = np.where(sd < float(eps), 1.0, sd).astype(np.float32, copy=False)
    return (x - mu) / sd
...
def build_hybrid_epochs_ddpm(
    ...
    *,
    ...
    """Reemplaza canales target por reconstrucción DDPM (opcional: per-epoch z-score)."""
    # IMPORTANT:
    ...
    for spec in specs:
        ...
        for s in range(0, n, gen_batch_size):
            ...
            # Stabilize synthesized channel only (avoid collapsing CSP/FBCSP/EEGNet on hybrid)
            if bool(per_epoch_z):
...
def run_csp_lda(
    *,
    ...
    # Keep CSP components fixed to 4; add robust fallback if primary produces NaNs/Infs.
...
def run_fbcsp_lda(
    *,
    ...
    # Hard constraint: CSP components must be 4 => per_band=4 => m_pairs=2
    m_pairs = 2
    per_band = int(2 * m_pairs)  # features por banda

    # Best-known config (from 2a tuning): ledoit_wolf + cov_est=epoch, keep CSP=4
    CSP_CFG = dict(n_components=per_band, reg="ledoit_wolf", log=True, norm_trace=True, cov_est="epoch")
    LDA_CFG = dict(solver="lsqr", shrinkage="auto")
...
def main() -> None:
    ...
    if bool(args.use_official_test):
        _log_section(f"Load + preprocess (paper-like) + epoching (Subject {subject_id}: raw01+raw02+raw03 train, raw04 test + labels8 .asc)")
    else:
    ...
    #
    # Instead:
    # - build epochs from the filtered signal first (no ICA yet)
    ...
    x_filt = raw.get_data().T.astype(np.float32)  # (n_times, 32)
    ...
    if bool(args.use_official_test):
        ...
        # preprocess test with SAME filter (ICA will be fit on TRAIN epochs later)
        raw_test = mne.io.RawArray(x_test_raw.T, info, verbose=False)
        raw_test = raw_test.copy().filter(l_freq=8.0, h_freq=30.0, method="iir", verbose=False)
        x_test_filt = raw_test.get_data().T.astype(np.float32)
    ...
    def _ica_fit_apply_epochs(Xtr_raw: np.ndarray, Xother_raw: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        # We already bandpass-filtered the signal at 8–30 Hz before epoching.
        # Set info fields so MNE doesn't warn about missing high-pass metadata during ICA.
        try:
            info_ep["highpass"] = 8.0
            info_ep["lowpass"] = 30.0
        except Exception:
            pass
        ...
        ica_f.fit(ep_tr, verbose=False)
        try:
            eog_idx, _ = ica_f.find_bads_eog(ep_tr, ch_name=["Fp1", "Fp2"], verbose=False)
    ...
    # Full TRAIN preprocessing for held-out TEST:
    # - fit ICA on TRAIN only, apply to TRAIN+TEST
    ...
    if bool(args.only_eegnet):
        ...
        out_test = run_eegnet(
            ...
            X_test_hybrid=X_test_real,  # placeholder (hybrid not evaluated in this mode)
            y_test=y_test,
            device=device,
            epochs=int(args.cls_epochs),
            batch_size=int(args.cls_batch_size),
        )
        no_cv = out_test.get("no_cv", {})
        acc_real = float(no_cv.get("acc_real", float("nan")))
        kappa_real = float(no_cv.get("kappa_real", float("nan")))
    ...
    if not args.skip_ddpm:
        ...
        for fold_i, (tr_idx, va_idx) in enumerate(splits, start=1):
            ...
            out_csp = run_csp_lda(X_train=X_tr, y_train=y_tr, X_test_real=X_va_real, X_test_hybrid=X_va_hybrid, y_test=y_va)
            out_f = run_fbcsp_lda(
                X_train=X_tr,
                y_train=y_tr,
        ...
        # -------------------------------
        # 3) Ablation study (REAL vs PARTIAL vs FULL)
        # -------------------------------
        _log_section(f"=== ABLATION STUDY - DDPM SUBJECT {subject_id} ===")
        X_test_partial = build_partial_hybrid_epochs_ddpm(
            X_test_real,
            specs=EXPERIMENTS,
            trained_models=trained_models,
            trained_input_idxs=trained_input_idxs,
            ...
            sqrt_one_minus_alphas_cumprod=sqrt_one_minus_alphas_cumprod,
            device=device,
            replace_ratio=float(args.partial_hybrid_ratio),
            seed=int(args.seed),
            per_epoch_z=bool(args.hybrid_per_epoch_z),
        )
        ...
        for clf_name in ["CSP+LDA", "FBCSP+LDA", "EEGNet"]:
            ...
            print(clf_name)
            print("-" * 80)
            for scen_name, X_h in scenarios.items():
                if clf_name == "CSP+LDA":
                    out = run_csp_lda(X_train=X_train_full, y_train=y_train_full, X_test_real=X_test_real, X_test_hybrid=X_h, y_test=y_test)
                elif clf_name == "FBCSP+LDA":
                    out = run_fbcsp_lda(
                        X_train=X_train_full,
                        y_train=y_train_full,
                        X_test_real=X_test_real,
                    ...
                    # FAIR ABLATION: reuse the SAME trained EEGNet weights for all scenarios
                    m_r = m_real  # fixed-model metrics on TEST(real)
                    ...
                    no_cv = {
                        "acc_real": float(m_r["acc"]),
                        "kappa_real": float(m_r["kappa"]),
                ...
                print(
                    f"{scen_name:8s} | "
                    f"Acc Original={float(no_cv.get('acc_real', float('nan'))):.3f} | "
                    f"Acc Hybrid={float(no_cv.get('acc_hybrid', float('nan'))):.3f} | "
                    f"Kappa Original={float(no_cv.get('kappa_real', float('nan'))):.3f} | "
                    f"Kappa Hybrid={float(no_cv.get('kappa_hybrid', float('nan'))):.3f}"
                )
