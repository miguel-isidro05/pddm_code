...
def run_csp_lda(
    *,
    ...
    if decision_threshold is None:
        ...
        p_real = lda.predict_proba(F_test_real)[:, 1]
        p_hyb = lda.predict_proba(F_test_hybrid)[:, 1]
...
def run_fbcsp_lda(
    *,
    ...
    # Stable FBCSP config requested: Ledoit-Wolf + concat covariance.
    CSP_CFG_PRIMARY = dict(n_components=per_band, reg="ledoit_wolf", log=True, norm_trace=False, cov_est="concat")
    CSP_CFG_FALLBACK = dict(n_components=per_band, reg="oas", log=True, norm_trace=False, cov_est="concat")
    LDA_CFG = dict(solver="lsqr", shrinkage="auto")
