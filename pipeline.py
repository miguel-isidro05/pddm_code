...
def run_csp_lda(
    *,
    ...
    CSP_CFG_PRIMARY = dict(n_components=4, reg="ledoit_wolf", log=True, norm_trace=False, cov_est="concat")
    CSP_CFG_FALLBACK = dict(n_components=4, reg="oas", log=True, norm_trace=False, cov_est="concat")
...
def run_fbcsp_lda(
    *,
    ...
    # Primary: classic FBCSP (literature-style).
    # Fallback: regularized CSP to avoid LinAlgError when covariance is not PD.
    CSP_CFG_PRIMARY = dict(n_components=per_band, reg=None, log=True, norm_trace=True, cov_est="epoch")
    CSP_CFG_FALLBACK = dict(n_components=per_band, reg="ledoit_wolf", log=True, norm_trace=False, cov_est="concat")
    LDA_CFG = dict(solver="lsqr", shrinkage="auto")
    ...
    for (l_f, h_f) in bands:
        ...
        try:
            ...
            print(
                ...
                "retrying with regularized fallback."
            )
...
def run_eegnet(
    *,
    ...
    # Requested config: Adam, lr=1e-4, fixed LR
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="Fraction of TRAIN used in full-train held-out stage (0,1]. Use ~0.8 to align with CV train size.",
    )
    ...
    parser.add_argument("--cls-epochs", type=int, default=400, help="Epochs para EEGNet")
