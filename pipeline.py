...
# --------------------------------------------------------------------------------------
# Score-based model (VP-SDE) + EMA + PC sampler (reemplaza SGDM.py)
# --------------------------------------------------------------------------------------

def _sgm_swish(x: torch.Tensor) -> torch.Tensor:
    return x * torch.sigmoid(x)


def _sgm_best_group_count(num_channels: int, max_groups: int = 32) -> int:
...
def _sgm_group_norm(num_channels: int, max_groups: int = 32) -> nn.GroupNorm:
    return nn.GroupNorm(_sgm_best_group_count(num_channels, max_groups=max_groups), num_channels=num_channels)


class _SGMGaussianFourierProjection(nn.Module):
    def __init__(self, embed_dim: int, scale: float = 30.0):
        super().__init__()
        self.W = nn.Parameter(torch.randn(embed_dim // 2) * float(scale), requires_grad=False)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_proj = x[:, None] * self.W[None, :] * 2.0 * float(np.pi)
        return torch.cat([torch.sin(x_proj), torch.cos(x_proj)], dim=-1)


class _SGMDense1D(nn.Module):
...
class ScoreNetUNet1D(nn.Module):
    """ScoreNet-style 1D U-Net backbone (continuous t)."""

    def __init__(self, in_channels: int, out_channels: int, *, n_feat: int = 64, time_emb_dim: int = 256):
        super().__init__()
        self.embed = nn.Sequential(
            _SGMGaussianFourierProjection(embed_dim=time_emb_dim),
            nn.Linear(time_emb_dim, time_emb_dim),
        )

        self.conv1 = nn.Conv1d(in_channels, n_feat, kernel_size=3, padding=1, bias=False)
        self.dense1 = _SGMDense1D(time_emb_dim, n_feat)
        self.gnorm1 = _sgm_group_norm(n_feat)

        self.conv2 = nn.Conv1d(n_feat, n_feat * 2, kernel_size=3, stride=2, padding=1, bias=False)
        self.dense2 = _SGMDense1D(time_emb_dim, n_feat * 2)
        self.gnorm2 = _sgm_group_norm(n_feat * 2)

        self.conv3 = nn.Conv1d(n_feat * 2, n_feat * 4, kernel_size=3, stride=2, padding=1, bias=False)
        self.dense3 = _SGMDense1D(time_emb_dim, n_feat * 4)
        self.gnorm3 = _sgm_group_norm(n_feat * 4)

        self.conv4 = nn.Conv1d(n_feat * 4, n_feat * 8, kernel_size=3, stride=2, padding=1, bias=False)
        self.dense4 = _SGMDense1D(time_emb_dim, n_feat * 8)
        self.gnorm4 = _sgm_group_norm(n_feat * 8)

        self.tconv4 = nn.ConvTranspose1d(n_feat * 8, n_feat * 4, kernel_size=4, stride=2, padding=1, bias=False)
        self.dense5 = _SGMDense1D(time_emb_dim, n_feat * 4)
        self.tgnorm4 = _sgm_group_norm(n_feat * 4)

        self.tconv3 = nn.ConvTranspose1d(n_feat * 8, n_feat * 2, kernel_size=4, stride=2, padding=1, bias=False)
        self.dense6 = _SGMDense1D(time_emb_dim, n_feat * 2)
        self.tgnorm3 = _sgm_group_norm(n_feat * 2)

        self.tconv2 = nn.ConvTranspose1d(n_feat * 4, n_feat, kernel_size=4, stride=2, padding=1, bias=False)
        self.dense7 = _SGMDense1D(time_emb_dim, n_feat)
        self.tgnorm2 = _sgm_group_norm(n_feat)

        self.tconv1 = nn.Conv1d(n_feat * 2, out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        emb = _sgm_swish(self.embed(t))

        h1 = self.conv1(x)
        h1 = _sgm_swish(self.gnorm1(h1 + self.dense1(emb)))

        h2 = self.conv2(h1)
        h2 = _sgm_swish(self.gnorm2(h2 + self.dense2(emb)))

        h3 = self.conv3(h2)
        h3 = _sgm_swish(self.gnorm3(h3 + self.dense3(emb)))

        h4 = self.conv4(h3)
        h4 = _sgm_swish(self.gnorm4(h4 + self.dense4(emb)))

        h = self.tconv4(h4)
        h = _sgm_swish(self.tgnorm4(h + self.dense5(emb)))

        h = self.tconv3(torch.cat([h, h3], dim=1))
        h = _sgm_swish(self.tgnorm3(h + self.dense6(emb)))

        h = self.tconv2(torch.cat([h, h2], dim=1))
        h = _sgm_swish(self.tgnorm2(h + self.dense7(emb)))
    ...
    *,
    ...
    for t in t_seq:
        ...
        for _ in range(int(n_steps_each)):
            ...
            step_size = (float(snr) * float(noise_norm) / (float(grad_norm) + 1e-12)) ** 2 * 2.0
            # Cap para evitar explosiones cuando grad_norm ~ 0 al inicio
            step_size = float(min(step_size, 0.1))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Backbone para score model. scorenet-unet usa un UNet estilo ScoreNet (incluido en este script).",
    )
    ...
    if not args.skip_ddpm:
        ...
        print(f"[U-Net cfg] epochs={int(args.cls_epochs)} | batch_size={int(args.cls_batch_size)} | optimizer=Adam | lr=1e-3")
    ...
    if not args.skip_sgm:
        ...
        # Importante: el modelo (UNet) usa time-embedding estilo DDPM, así que
        # mantenemos la escala de embedding consistente con el entrenamiento (N=1000)
        # incluso si el sampling usa menos pasos (sampling_N).
        sde_train = VPSDE(beta_min=0.1, beta_max=20.0, N=1000, T=1.0)
        sde_sampling = VPSDE(beta_min=0.1, beta_max=20.0, N=1000, T=1.0)
        ...
        for spec in EXPERIMENTS:
            ...
            if args.sgm_backbone == "scorenet-unet":
                model = ScoreNetUNet1D(in_channels=3, out_channels=1, n_feat=64, time_emb_dim=256).to(device)
                t_embed_scale = None  # ScoreNet UNet expects continuous t
            else:
                model = UNet(in_channels=3, out_channels=1, n_feat=64, time_emb_dim=256).to(device)
            ...
            if args.sgm_backbone == "scorenet-unet":
                ema_model = ScoreNetUNet1D(in_channels=3, out_channels=1, n_feat=64, time_emb_dim=256).to(device)
            else:
