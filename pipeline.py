...
def main() -> None:
    ...
    if not args.skip_sgm:
        ...
        for spec in EXPERIMENTS:
            ...
            ckpt_stem = f"sgm_{sde_name}_{args.sgm_backbone}_{spec.target}_steps{int(args.sgm_train_steps)}"
            ckpt_path_pth = ckpt_dir / f"{ckpt_stem}.pth"
            ckpt_path_pt = ckpt_dir / f"{ckpt_stem}.pt"  # backward-compat
            ...
            ckpt_path = ckpt_path_pth if ckpt_path_pth.exists() else ckpt_path_pt
            if bool(args.sgm_cache) and (not bool(args.sgm_force_train)) and ckpt_path.exists():
                try:
                    ckpt = torch.load(ckpt_path, map_location=device)
                    model.load_state_dict(ckpt["model_state"], strict=True)
                    loaded_from_ckpt = True
                    print(f"[SGM ckpt] loaded: {ckpt_path.name}")
                except Exception as e:
                    print(f"[SGM ckpt] load failed ({ckpt_path.name}): {e}. Will train from scratch.")
                    ...
                    torch.save(
                        {
                            ...
                            "meta": {
                                ...
                                "input_idxs": tuple(int(v) for v in input_idxs),
                                "backbone": str(args.sgm_backbone),
                                "sde": sde_name,
                                "train_steps": int(args.sgm_train_steps),
                                "ve_sigma": float(ve_sigma),
                                "vp_beta_max": float(args.sgm_vp_beta_max),
                                "seed": int(args.seed),
                            },
                        },
                        ckpt_path_pth,
                    )
                    print(f"[SGM ckpt] saved: {ckpt_path_pth.name}")
                except Exception as e:
                    print(f"[SGM ckpt] save failed ({ckpt_path_pth.name}): {e}")
