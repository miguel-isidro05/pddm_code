...
def _f1_left_right(y_true_enc: np.ndarray, y_pred_enc: np.ndarray) -> tuple[float, float]:
    """
    Return class-wise F1 for encoded binary labels:
    - class 0 -> Left
    - class 1 -> Right
    If labels are not binary, returns NaN values.
    """
    from sklearn.metrics import f1_score

    y_true_enc = np.asarray(y_true_enc, dtype=int).reshape(-1)
    y_pred_enc = np.asarray(y_pred_enc, dtype=int).reshape(-1)
    labels_present = np.unique(np.concatenate([y_true_enc, y_pred_enc])) if y_true_enc.size > 0 else np.asarray([], dtype=int)
    if int(labels_present.size) < 2:
        return float("nan"), float("nan")
    f1_lr = f1_score(y_true_enc, y_pred_enc, labels=[0, 1], average=None, zero_division=0)
    if int(np.asarray(f1_lr).size) < 2:
        return float("nan"), float("nan")
    return float(f1_lr[0]), float(f1_lr[1])
...
def run_csp_lda(
    *,
    ...
    f1_l_real, f1_r_real = _f1_left_right(y_test_enc, pred_real)
    f1_l_h, f1_r_h = _f1_left_right(y_test_enc, pred_hybrid)
...
def run_fbcsp_lda(
    *,
    ...
    f1_l_real, f1_r_real = _f1_left_right(y_test_enc, pred_real)
    f1_l_h, f1_r_h = _f1_left_right(y_test_enc, pred_h)
...
def run_eegnet(
    *,
    ...
    def _eval(x: np.ndarray, y: np.ndarray) -> dict[str, float]:
        ...
        f1_l, f1_r = _f1_left_right(y, y_pred)
    ...
    return {
        "no_cv": {
            ...
            "f1_l_real": float(m_real["f1_l"]),
            "f1_r_real": float(m_real["f1_r"]),
            "f1_l_hybrid": float(m_h["f1_l"]),
            "f1_r_hybrid": float(m_h["f1_r"]),
        },
        "cv": {},
    }
...
def _eval_eegnet_fixed(
    *,
    ...
    f1_l, f1_r = _f1_left_right(yte, y_pred)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Run all LOSO folds (each subject held-out once) and print mean±std summary.",
    )
    parser.add_argument(
        "--use-official-test",
        action=argparse.BooleanOptionalAction,
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Skip TRAIN-only CV in the full DDPM pipeline and report held-out TEST only.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--generate-ddpm-figures",
        action="store_true",
        help="Generate DDPM paper figures only (trains DDPM for figure channels, skips full classifier pipeline).",
    )
    parser.add_argument(
        "--ddpm-all-subjects",
        action=argparse.BooleanOptionalAction,
        default=False,
        help=(
            "Run DDPM-only pipeline sequentially for subjects 1,2,3 with fixed "
            "settings (seed=0, cv-seed=42, lr-only, skip-sgm), print per-subject "
            "tables and final mean±std + Wilcoxon."
        ),
    )
    args = parser.parse_args()
    # Hard override hook (used by ddpm-all-subjects child runs).
    if os.environ.get("PIPELINE_FORCE_NO_CV_GLOBAL", "").strip() == "1":
        args.no_cv_global = True

    if bool(args.ddpm_all_subjects):
        _log_section("DDPM-ALL-SUBJECTS mode: subjects [1,2,3], lr-only, seed=0, cv-seed=42, no score-based")
        t_all_start = time.time()

        def _remove_arg_with_value(argv: list[str], key: str) -> list[str]:
            out: list[str] = []
            skip_next = False
            for tok in argv:
                if skip_next:
        ...
        for flag in (
            ...
            "--cv-seed",
            "--use-official-test",
            "--no-use-official-test",
            "--no-cv-global",
            "--no-no-cv-global",
        ):
            if flag in base_argv:
                base_argv = [a for a in base_argv if a != flag]
        base_argv = _remove_arg_with_value(base_argv, "--subject")
        base_argv = _remove_arg_with_value(base_argv, "--seed")
        base_argv = _remove_arg_with_value(base_argv, "--cv-seed")
        forced_args = [
            "--lr-only",
            "--skip-sgm",
            "--seed",
        ...
        line_re = re.compile(
            r"^FINAL_METRIC\|(\d+)\|(CSP\+LDA|FBCSP\+LDA|EEGNet)\|(Original|Hybrid)\|([0-9eE+\-\.]+)\|([0-9eE+\-\.]+)\|([0-9eE+\-\.]+)\|([0-9eE+\-\.]+)$",
            re.MULTILINE,
        )
        metrics_by_model: dict[str, dict[str, dict[str, list[float]]]] = {
            m: {
                "Original": {"acc": [], "kappa": [], "f1_l": [], "f1_r": []},
                "Hybrid": {"acc": [], "kappa": [], "f1_l": [], "f1_r": []},
            }
            for m in ("CSP+LDA", "FBCSP+LDA", "EEGNet")
        }

        for sid in (1, 2, 3):
            t_sid_start = time.time()
            cmd = [sys.executable, str(Path(__file__).resolve())] + base_argv + forced_args + ["--subject", str(int(sid))]
            print(f"\n[DDPM-ALL-SUBJECTS] Running subject {sid}: {' '.join(cmd)}\n")
            child_env = os.environ.copy()
            child_env["PIPELINE_FORCE_NO_CV_GLOBAL"] = "1"
            ...
            if return_code != 0:
                raise RuntimeError(f"DDPM subject run failed for subject {sid} with code {return_code}")
            elapsed_sid = float(time.time() - t_sid_start)
            print(f"[DDPM-ALL-SUBJECTS] Subject {sid} elapsed time: {elapsed_sid:.1f}s ({elapsed_sid/60.0:.2f} min)")

            rows = list(line_re.finditer(out_text))
            for m in rows:
                model = str(m.group(2))
                cond = str(m.group(3))
                acc = float(m.group(4))
                kappa = float(m.group(5))
                f1_l = float(m.group(6))
                f1_r = float(m.group(7))
                if model in metrics_by_model and cond in metrics_by_model[model]:
                    metrics_by_model[model][cond]["acc"].append(acc)
                    metrics_by_model[model][cond]["kappa"].append(kappa)
                    metrics_by_model[model][cond]["f1_l"].append(f1_l)
                    metrics_by_model[model][cond]["f1_r"].append(f1_r)
        ...
        print("\n" + "=" * 80)
        print("Table 1 (mean across subjects) — DDPM-only")
        print("=" * 80)
        rows_out: list[dict[str, object]] = []
        for model in ("CSP+LDA", "FBCSP+LDA", "EEGNet"):
            for cond in ("Original", "Hybrid"):
                m_acc, s_acc = _mean_std(metrics_by_model[model][cond]["acc"])
                m_k, s_k = _mean_std(metrics_by_model[model][cond]["kappa"])
                m_f1_l, _ = _mean_std(metrics_by_model[model][cond]["f1_l"])
                m_f1_r, _ = _mean_std(metrics_by_model[model][cond]["f1_r"])
                rows_out.append(
                    {
                        "Method": model,
                        "Condition": cond,
                        "Accuracy": f"{m_acc:.3f} ± {s_acc:.3f}" if np.isfinite(m_acc) and np.isfinite(s_acc) else "nan",
                        "κ": f"{m_k:.3f} ± {s_k:.3f}" if np.isfinite(m_k) and np.isfinite(s_k) else "nan",
                        "F1-L": m_f1_l,
                        "F1-R": m_f1_r,
                    }
                )
        ...
        def _wilcoxon_pvals(a: list[float], b: list[float]) -> float:
            ...
            if int(xa.size) < 2:
                return float("nan")
            try:
                _, p = wilcoxon(xa, xb, zero_method="wilcox", alternative="two-sided")
                return float(p)
            except Exception:
                return float("nan")

        print("\nWilcoxon (Original vs Hybrid):")
        for model in ("CSP+LDA", "FBCSP+LDA", "EEGNet"):
            p_acc = _wilcoxon_pvals(metrics_by_model[model]["Original"]["acc"], metrics_by_model[model]["Hybrid"]["acc"])
            p_k = _wilcoxon_pvals(metrics_by_model[model]["Original"]["kappa"], metrics_by_model[model]["Hybrid"]["kappa"])
            p_f1_l = _wilcoxon_pvals(metrics_by_model[model]["Original"]["f1_l"], metrics_by_model[model]["Hybrid"]["f1_l"])
            p_f1_r = _wilcoxon_pvals(metrics_by_model[model]["Original"]["f1_r"], metrics_by_model[model]["Hybrid"]["f1_r"])
            print(f"{model}:")
            print(f"Accuracy p = {p_acc:.6g}" if np.isfinite(p_acc) else "Accuracy p = nan")
            print(f"κ p = {p_k:.6g}" if np.isfinite(p_k) else "κ p = nan")
            print(f"F1-L p = {p_f1_l:.6g}" if np.isfinite(p_f1_l) else "F1-L p = nan")
            print(f"F1-R p = {p_f1_r:.6g}" if np.isfinite(p_f1_r) else "F1-R p = nan")
        elapsed_all = float(time.time() - t_all_start)
        print(f"\n[DDPM-ALL-SUBJECTS] Total elapsed time: {elapsed_all:.1f}s ({elapsed_all/60.0:.2f} min)")
        print("=" * 80 + "\n")
        return

    if bool(args.loso_all):
        loso_subjects_cli = [int(s.strip()) for s in str(args.loso_subjects).split(",") if str(s).strip()]
        if len(loso_subjects_cli) == 0:
            raise ValueError("--loso-subjects is empty.")
        loso_subjects_cli = sorted(set(loso_subjects_cli))
        if any(s not in (1, 2, 3) for s in loso_subjects_cli):
            raise ValueError(f"--loso-subjects must contain IDs in [1,2,3], got: {loso_subjects_cli}")

        _log_section(f"LOSO-ALL mode: running folds for held-out subjects {loso_subjects_cli}")
        ...
        base_argv = list(sys.argv[1:])
        base_argv = [a for a in base_argv if a not in ("--loso-all", "--no-loso-all")]
        base_argv = _remove_arg_with_value(base_argv, "--subject")
        # Force LOSO fold behavior in child runs.
        if "--loso" not in base_argv:
            base_argv.append("--loso")
        if ("--use-official-test" not in base_argv) and ("--no-use-official-test" not in base_argv):
            base_argv.append("--use-official-test")
        ...
        for held_out_sid in loso_subjects_cli:
            cmd = [sys.executable, str(Path(__file__).resolve())] + base_argv + ["--subject", str(int(held_out_sid))]
            print(f"\n[LOSO-ALL] Running held-out subject {held_out_sid}: {' '.join(cmd)}\n")
            proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            print(proc.stdout)
            if int(proc.returncode) != 0:
                raise RuntimeError(f"LOSO fold failed for held-out subject {held_out_sid} with code {proc.returncode}")

            rows = list(line_re.finditer(proc.stdout))
            for m in rows:
                model = str(m.group(2))
                cond = str(m.group(3))
                acc = float(m.group(4))
        ...
        # Table 1 across subjects (mean values only; std can be derived across subjects if needed).
        print("\n" + "=" * 80)
        print("Table 1 (mean across subjects)")
        print("=" * 80)
        rows_out: list[dict[str, object]] = []
        for model in ("CSP+LDA", "FBCSP+LDA", "EEGNet"):
            for cond in ("Original", "Hybrid"):
        ...
        # Paired Wilcoxon across subjects: Original vs Hybrid, per method.
        def _wilcoxon_pvals(a: list[float], b: list[float]) -> float:
            from scipy.stats import wilcoxon
    ...
    def _generate_ddpm_paper_figures() -> None:
        """
        Generate paper figures using REAL DDPM behavior on selected channels only.
        This mode trains DDPM only for figure channels and skips classifier stages.
        """
        import matplotlib.pyplot as plt
        import mne
        from scipy.signal import fftconvolve

        out_dir = Path("figures_ddpm")
        out_dir.mkdir(parents=True, exist_ok=True)

        # Reuse existing fold-safe preprocessing path (ICA fit on TRAIN only + 8-30 + train-zscore).
        X_train_fig, X_test_real = _prepare_full_train_test_preproc()

        wanted = ["AF3", "T8"]
        for ch in wanted:
            if ch not in CHANNEL_TO_IDX:
                raise ValueError(f"Channel {ch} not found in CHANNEL_TO_IDX.")
        ch_idxs = [CHANNEL_TO_IDX[ch] for ch in wanted]
        sf = float(sfreq)
        n_t = int(X_test_real.shape[2])
        t_sec = np.arange(n_t, dtype=np.float64) / sf

        # ------------------------------
        # Train DDPM only for requested channels and build true DDPM hybrid.
        # ------------------------------
        rng = np.random.default_rng(int(args.seed))
        timesteps_fig = 1000
        betas_t = linear_beta_schedule(timesteps_fig).to(device)
        ...
        betas_fig = betas_t.detach().cpu().numpy().astype(np.float64)
        alphas_cumprod_fig = alphas_cumprod_t.detach().cpu().numpy().astype(np.float64)

        spec_by_target = {spec.target: spec for spec in EXPERIMENTS}
        figure_specs: list[ExperimentSpec] = []
        for ch in wanted:
            if ch not in spec_by_target:
                raise ValueError(f"Target {ch} is not configured in EXPERIMENTS.")
            figure_specs.append(spec_by_target[ch])

        _log_section("Figure mode: train DDPM for channels [AF3, T8]")
        ddpm_models_fig: dict[str, nn.Module] = {}
        ddpm_inputs_fig: dict[str, tuple[int, int]] = {}
        for spec in figure_specs:
            input_idxs = (CHANNEL_TO_IDX[spec.inputs[0]], CHANNEL_TO_IDX[spec.inputs[1]])
            target_idx = CHANNEL_TO_IDX[spec.target]
            train_ds = EEGEpochs(X_train_fig, input_idxs=input_idxs, target_idx=target_idx)
            ...
            train_diffusion_model(
                ...
                device=device,
                epoch_desc=f"DDPM Figure Train {spec.target}",
            )
            ddpm_models_fig[spec.target] = model.eval()
            ddpm_inputs_fig[spec.target] = input_idxs

        x_hybrid = np.asarray(X_test_real, dtype=np.float32).copy()
        gen_bs = int(args.gen_batch_size)
        for spec in figure_specs:
            model = ddpm_models_fig[spec.target]
            input_idxs = ddpm_inputs_fig[spec.target]
            target_idx = CHANNEL_TO_IDX[spec.target]
            for start in range(0, int(x_hybrid.shape[0]), gen_bs):
                stop = min(start + gen_bs, int(x_hybrid.shape[0]))
                cond_np = np.asarray(X_test_real[start:stop, list(input_idxs), :], dtype=np.float32)
                ...
                with torch.no_grad():
                    syn = sample_from_model(
                        ...
                        betas_t,
                        sqrt_one_minus_alphas_cumprod_t,
                        shape=(int(stop - start), 1, int(n_t)),
                        device=device,
                    )
                x_hybrid[start:stop, target_idx, :] = syn[:, 0, :].detach().cpu().numpy().astype(np.float32)
        ...
        def _sample_with_trajectory(
            ...
            *,
            ...
            model.eval()
            bsz = int(conditioning_inputs.shape[0])
            x = torch.randn((bsz, 1, int(n_t)), device=device)
            keep = set(int(v) for v in trajectory_steps)
            with torch.no_grad():
                for t in reversed(range(timesteps_fig)):
                    t_batch = torch.full((bsz,), int(t), device=device, dtype=torch.long)
                    beta_t = betas_t[t]
                    alpha_t = alphas_t[t]
                    alpha_cumprod_ti = alphas_cumprod_t[t]
                    sqrt_one_minus_alpha_cumprod_t = torch.sqrt(1.0 - alpha_cumprod_ti)
                    sqrt_alpha_t = torch.sqrt(alpha_t)
                    model_input = torch.cat([conditioning_inputs, x], dim=1)
                    noise_pred = model(model_input, t_batch)
                    x = (1.0 / sqrt_alpha_t) * (x - (beta_t / sqrt_one_minus_alpha_cumprod_t) * noise_pred)
                    if int(t) in keep:
                        out[int(t)] = x[0, 0, :].detach().cpu().numpy().astype(np.float64)
            return out

        # ------------------------------
        # Figure 2: forward/reverse snapshots at t={0,500,999} with temporal(0-0.5s).
        # ------------------------------
        fig2_dir = out_dir / "figure2_forward_reverse"
        fig2_dir.mkdir(parents=True, exist_ok=True)
        sample_idx = int(min(20, max(0, int(X_test_real.shape[0]) - 1)))
        t_forward = 200
        t_forward_minus_1 = 999
        t_reverse = 800
        t_grid_forward = [0, int(t_forward), int(t_forward_minus_1)]  # t0, t, t-1
        reverse_steps = [int(t_reverse), 0]  # t, t0
        n_half = max(1, int(round(0.5 * sf)))
        t_half = t_sec[:n_half]
        color_forward_default = "black"
        color_forward_tminus1 = "#d62728"  # red
        color_reverse = "#1f77b4"  # blue

        def _time_tag_forward(tt: int) -> str:
            if int(tt) == 0:
                return "t0"
            if int(tt) == int(t_forward):
                return "t"
            if int(tt) == int(t_forward_minus_1):
                return "t_minus_1"
            return f"t{int(tt)}"

        def _time_tag_reverse(tt: int) -> str:
            if int(tt) == 0:
                return "t0"
            if int(tt) == int(t_reverse):
                return "t"
            return f"t{int(tt)}"

        for ch_name, ch_idx in zip(wanted, ch_idxs):
            x0 = np.asarray(X_test_real[sample_idx, ch_idx, :], dtype=np.float64)
            eps = rng.standard_normal(size=x0.shape)

            # Reverse iterations from the actual DDPM model.
            spec = spec_by_target[ch_name]
            cond_idxs = ddpm_inputs_fig[spec.target]
            cond_np = np.asarray(X_test_real[sample_idx : sample_idx + 1, list(cond_idxs), :], dtype=np.float32)
            cond_t = torch.from_numpy(cond_np).to(device)
            reverse_series = _sample_with_trajectory(ddpm_models_fig[spec.target], cond_t, trajectory_steps=reverse_steps)

            # Save one image per t for forward: temporal (0-0.5s).
            for tt in t_grid_forward:
                ab = float(alphas_cumprod_fig[int(tt)])
                x_forward = np.sqrt(max(ab, 1e-12)) * x0 + np.sqrt(max(1.0 - ab, 1e-12)) * eps
                color_forward = color_forward_tminus1 if int(tt) == int(t_forward_minus_1) else color_forward_default
                fig_f_t, ax_f_t = plt.subplots(1, 1, figsize=(6.2, 3.6), facecolor="white")
                ax_f_t.set_facecolor("white")
                ax_f_t.plot(t_half, x_forward[:n_half], color=color_forward, lw=1.2, alpha=0.95)
                ax_f_t.grid(False, which="both")
                ax_f_t.set_xticks([])
                ax_f_t.set_yticks([])
                fig_f_t.tight_layout()
                tt_name = _time_tag_forward(int(tt))
                fig_f_t.savefig(
                    fig2_dir / f"figure2_{ch_name.lower()}_forward_t{tt_name}_temporal.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close(fig_f_t)

            # Save one image per t for reverse: temporal (0-0.5s).
            for tt in reverse_steps:
                x_rev = reverse_series.get(int(tt))
                if x_rev is None:
                    continue
                fig_r_t, ax_r_t = plt.subplots(1, 1, figsize=(6.2, 3.6), facecolor="white")
                ax_r_t.set_facecolor("white")
                ax_r_t.plot(t_half, x_rev[:n_half], color=color_reverse, lw=1.2, alpha=0.95)
                ax_r_t.grid(False, which="both")
                ax_r_t.set_xticks([])
                ax_r_t.set_yticks([])
                fig_r_t.tight_layout()
                tt_name = _time_tag_reverse(int(tt))
                fig_r_t.savefig(
                    fig2_dir / f"figure2_{ch_name.lower()}_reverse_t{tt_name}_temporal.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close(fig_r_t)
        ...
        # ------------------------------
        # Figure 3: Original vs Hybrid (temporal, wavelet) for AF3 and T8.
        # ------------------------------
        # Build MNE EpochsArray with events to follow the notebook-style analysis flow.
        info_2ch = mne.create_info(ch_names=wanted, sfreq=sf, ch_types="eeg")
        y_ev = np.asarray(labels_test, dtype=np.int64).reshape(-1)
        events_2ch = np.column_stack(
            [np.arange(int(y_ev.shape[0]), dtype=np.int64), np.zeros(int(y_ev.shape[0]), dtype=np.int64), y_ev]
        ).astype(np.int64)
        event_id_2ch = {f"class_{int(k)}": int(k) for k in np.unique(y_ev).tolist()}
        ep_real_2ch = mne.EpochsArray(
            X_test_real[:, ch_idxs, :], info_2ch, events=events_2ch, event_id=event_id_2ch, tmin=0.0, verbose=False
        )
        ep_hyb_2ch = mne.EpochsArray(
            x_hybrid[:, ch_idxs, :], info_2ch, events=events_2ch, event_id=event_id_2ch, tmin=0.0, verbose=False
        )

        # Wavelet TFR (custom Morlet + fftconvolve), matching manuscript-style visualization.
        def _morlet_tfr_epochs_power(
            X_ep: np.ndarray,
            fs_hz: float,
            *,
            fmin: float = 8.0,
            ...
            n_freqs: int = 40,
            n_cycles: float = 6.0,
            wavelet_support_s: float = 2.0,
        ) -> tuple[np.ndarray, np.ndarray]:
            """
            X_ep shape: (n_trials, n_samples). Returns:
            - freqs: (n_freqs,)
            - power_mean: (n_freqs, n_samples)
            """
            X_ep = np.asarray(X_ep, dtype=np.float64)
            if X_ep.ndim != 2:
                raise ValueError(f"Expected X_ep 2D (n_trials, n_samples), got {X_ep.shape}")
            n_trials, n_samples = X_ep.shape
            if n_trials <= 0 or n_samples <= 0:
                raise ValueError("Empty input for Morlet TFR.")

            freqs = np.linspace(float(fmin), float(fmax), int(n_freqs))
            t_w = np.arange(-float(wavelet_support_s), float(wavelet_support_s), 1.0 / float(fs_hz), dtype=np.float64)
            power_accum = np.zeros((int(freqs.shape[0]), int(n_samples)), dtype=np.float64)

            for i_f, f in enumerate(freqs.tolist()):
                sigma = float(n_cycles) / (2.0 * np.pi * float(f))
                # Morlet kernel: complex sinusoid modulated by a Gaussian envelope.
                wavelet = np.exp(2j * np.pi * float(f) * t_w) * np.exp(-(t_w**2) / (2.0 * sigma**2))
                for tr in range(int(n_trials)):
                    z = fftconvolve(X_ep[tr], wavelet, mode="same")
                    power_accum[i_f] += np.abs(z) ** 2

            power_mean = power_accum / float(n_trials)
            return freqs, power_mean

        seg_epochs = int(max(1, round(4.0 * sf / max(1, n_t))))  # typically 4 when n_t=512, sf=512
        seg_epochs = min(seg_epochs, int(X_test_real.shape[0]))
        if seg_epochs <= 0:
            raise ValueError("Not enough test epochs to build 4-second wavelet segment.")
        start_idx4 = int(min(sample_idx, max(0, int(X_test_real.shape[0]) - int(seg_epochs))))
        idx4 = np.arange(start_idx4, start_idx4 + seg_epochs, dtype=int)
        x4_real = np.concatenate([X_test_real[i, ch_idxs, :] for i in idx4.tolist()], axis=1)  # (2, ~4s)
        x4_hyb = np.concatenate([x_hybrid[i, ch_idxs, :] for i in idx4.tolist()], axis=1)  # (2, ~4s)
        x4_real = x4_real.astype(np.float32)
        x4_hyb = x4_hyb.astype(np.float32)
        times_4s = np.arange(int(x4_real.shape[1]), dtype=np.float64) / float(sf)

        # Save separate scientific figures per channel and condition.
        fig3_dir = out_dir / "figure3_original_vs_hybrid"
        fig3_dir.mkdir(parents=True, exist_ok=True)
        seg_idx = int(sample_idx)
        for row_i, ch_name in enumerate(wanted):
            # Temporal: one image per condition.
            for cond_name, x_arr, color in [
                ("original", X_test_real, "black"),
                ("hybrid", x_hybrid, "#4c72b0"),
            ]:
                fig_t, ax_t = plt.subplots(1, 1, figsize=(10, 3.5), facecolor="white")
                ax_t.set_facecolor("white")
                ax_t.plot(t_sec, x_arr[seg_idx, ch_idxs[row_i], :], color=color, lw=1.2)
                ax_t.set_title(f"{ch_name} — Temporal ({cond_name})")
                ax_t.set_xlabel("Time (s)")
                ax_t.set_ylabel("Amplitude (a.u.)")
                ax_t.grid(False, which="both")
                fig_t.tight_layout()
                fig_t.savefig(fig3_dir / f"figure3_{ch_name.lower()}_temporal_{cond_name}.png", dpi=300, bbox_inches="tight")
                plt.close(fig_t)

            # Wavelet heatmap: one image per condition (MNE tfr_morlet).
            xo = np.asarray(x4_real[row_i], dtype=np.float64)[None, :]  # (1, T4)
            xd = np.asarray(x4_hyb[row_i], dtype=np.float64)[None, :]   # (1, T4)
            freqs_w, pwr_o = _morlet_tfr_epochs_power(
                xo,
                float(sf),
                fmin=8.0,
                fmax=30.0,
                n_freqs=40,
                n_cycles=6.0,
                wavelet_support_s=2.0,
            )
            _, pwr_d = _morlet_tfr_epochs_power(
                xd,
                float(sf),
                fmin=8.0,
                fmax=30.0,
            ...
            for cond_name, pwr in [("original", pwr_o), ("hybrid", pwr_d)]:
                fig_w, ax_w = plt.subplots(1, 1, figsize=(7.5, 4.5), facecolor="white")
                ax_w.set_facecolor("white")
                im = ax_w.imshow(
                    np.log10(np.maximum(pwr, 1e-12)),
                    aspect="auto",
                    origin="lower",
                ...
                ax_w.grid(False, which="both")
                plt.colorbar(im, ax=ax_w, fraction=0.046, pad=0.04, label="log10 Power")
                fig_w.tight_layout()
                fig_w.savefig(fig3_dir / f"figure3_{ch_name.lower()}_wavelet_{cond_name}.png", dpi=300, bbox_inches="tight")
                plt.close(fig_w)

        # Pearson summary between real and synthetic channels on held-out TEST (figure channels only).
        pearson_rows: list[tuple[str, float]] = []
        for ch_name, ch_idx in zip(wanted, ch_idxs):
            corr_vals = [
                float(pearson_corr_1d(X_test_real[i, ch_idx, :], x_hybrid[i, ch_idx, :]))
                for i in range(int(X_test_real.shape[0]))
            ]
            corr_mean = float(np.mean(corr_vals)) if corr_vals else float("nan")
            pearson_rows.append((ch_name, corr_mean))
            print(f"[figures] Pearson {ch_name} (real vs synthetic): {corr_mean:.6f}")

        pearson_path = out_dir / "figure_channels_pearson.txt"
        with pearson_path.open("w", encoding="utf-8") as f:
            f.write("channel\tpearson_mean\n")
            for ch_name, corr_mean in pearson_rows:
                f.write(f"{ch_name}\t{corr_mean:.6f}\n")
        print(f"[figures] saved pearson summary: {str(pearson_path.resolve())}")

        print(f"[figures] saved: {str(out_dir.resolve())}")

    if bool(args.generate_ddpm_figures):
        _log_section("Figure-only mode: generate DDPM paper figures (DDPM-on-figure-channels only)")
        _generate_ddpm_paper_figures()
        _log_section("Done")
        return
    ...
    if not args.skip_ddpm:
        ...
        y_test = labels_test
        print(f"[config] no_cv_global={bool(args.no_cv_global)}")

        cv_summary: dict[str, dict[str, float]] = {}
        if not bool(args.no_cv_global):
            # -------------------------------
            ...
            _log_section("DDPM - global CV skipped (--no-cv-global); reporting held-out TEST only")
        ...
        ddpm_acc_summary["EEGNet"] = {
            "no_cv": {
                ...
                "kappa_real": float(m_real["kappa"]),
                "kappa_hybrid": float(m_h["kappa"]),
                "f1_l_real": float(m_real.get("f1_l", float("nan"))),
                "f1_r_real": float(m_real.get("f1_r", float("nan"))),
                "f1_l_hybrid": float(m_h.get("f1_l", float("nan"))),
                "f1_r_hybrid": float(m_h.get("f1_r", float("nan"))),
            },
            "cv": {},
        }

        # Final table: held-out TEST metrics
        print("\n" + "=" * 80)
        print(f"DDPM - SUBJECT {subject_id} — Final summary (held-out TEST)")
        print("=" * 80)
        rows = []
            ...
            rows.append(
                {
                    "Method": name,
                    "Condition": "Original",
                    "Accuracy": float(no_cv.get("acc_real", float("nan"))),
                    "κ": float(no_cv.get("kappa_real", float("nan"))),
                    "F1-L": float(no_cv.get("f1_l_real", float("nan"))),
                    "F1-R": float(no_cv.get("f1_r_real", float("nan"))),
                }
            )
            rows.append(
                {
                    "Method": name,
                    "Condition": "Hybrid",
                    "Accuracy": float(no_cv.get("acc_hybrid", float("nan"))),
                    "κ": float(no_cv.get("kappa_hybrid", float("nan"))),
                    "F1-L": float(no_cv.get("f1_l_hybrid", float("nan"))),
                    "F1-R": float(no_cv.get("f1_r_hybrid", float("nan"))),
                }
            )
        try:
            import pandas as pd
        ...
        # Machine-readable lines for LOSO-ALL aggregation.
        for name, out in ddpm_acc_summary.items():
            no_cv = out.get("no_cv", {})
            print(
                "FINAL_METRIC|"
                f"{int(subject_id)}|{name}|Original|"
                f"{float(no_cv.get('acc_real', float('nan'))):.10f}|"
                f"{float(no_cv.get('kappa_real', float('nan'))):.10f}|"
                f"{float(no_cv.get('f1_l_real', float('nan'))):.10f}|"
                f"{float(no_cv.get('f1_r_real', float('nan'))):.10f}"
            )
            print(
                "FINAL_METRIC|"
                f"{int(subject_id)}|{name}|Hybrid|"
                f"{float(no_cv.get('acc_hybrid', float('nan'))):.10f}|"
                f"{float(no_cv.get('kappa_hybrid', float('nan'))):.10f}|"
                f"{float(no_cv.get('f1_l_hybrid', float('nan'))):.10f}|"
                f"{float(no_cv.get('f1_r_hybrid', float('nan'))):.10f}"
            )
