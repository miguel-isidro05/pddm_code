...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Generate paper figures only (no DDPM/SGM/CSP/FBCSP/EEGNet training or evaluation).",
    )
    ...
    def _generate_ddpm_paper_figures() -> None:
        """
        Generate lightweight paper figures only, reusing loaded/preprocessed data.
        No training/evaluation is performed in this mode.
        """
        ...
        _, X_test_real = _prepare_full_train_test_preproc()
        ...
        # ------------------------------
        # Build a lightweight "hybrid" proxy (illustrative only, no model training).
        # ------------------------------
        rng = np.random.default_rng(int(args.seed))
        x_hybrid = np.asarray(X_test_real, dtype=np.float32).copy()
        timesteps_fig = 1000
        betas_fig = linear_beta_schedule(timesteps_fig).detach().cpu().numpy().astype(np.float64)
        alphas_fig = 1.0 - betas_fig
        alphas_cumprod_fig = np.cumprod(alphas_fig)
        t_hybrid = 350
        a_bar = float(alphas_cumprod_fig[t_hybrid])
        s_ab = float(np.sqrt(max(a_bar, 1e-12)))
        s_1m = float(np.sqrt(max(1.0 - a_bar, 1e-12)))

        for ch_idx in ch_idxs:
            x0 = np.asarray(x_hybrid[:, ch_idx, :], dtype=np.float64)
            eps = rng.standard_normal(size=x0.shape)
            x_t = s_ab * x0 + s_1m * eps
            # Slightly biased reverse estimate to keep a realistic gap versus original.
            eps_pred = 0.85 * eps
            x0_hat = (x_t - s_1m * eps_pred) / max(s_ab, 1e-12)
            x_hybrid[:, ch_idx, :] = x0_hat.astype(np.float32)
        ...
        for ch_name, ch_idx in zip(wanted, ch_idxs):
            ...
            # Reverse illustration: from a noisy state towards reconstructed state.
            t_start = 700
            ab_s = float(alphas_cumprod_fig[int(t_start)])
            x_t_start = np.sqrt(max(ab_s, 1e-12)) * x0 + np.sqrt(max(1.0 - ab_s, 1e-12)) * eps
            eps_pred = 0.85 * eps
            x_rec = (x_t_start - np.sqrt(max(1.0 - ab_s, 1e-12)) * eps_pred) / max(np.sqrt(max(ab_s, 1e-12)), 1e-12)

            rev_lambdas = [1.00, 0.75, 0.50, 0.25, 0.00]
            colors_rev = plt.cm.plasma(np.linspace(0.15, 0.95, len(rev_lambdas)))
            fig_r, ax_r = plt.subplots(1, 1, figsize=(10, 3.5))
            ax_r.plot(t_sec, x0, color="black", lw=1.2, alpha=0.9, label="original")
            for c, lam in zip(colors_rev, rev_lambdas):
                # Visualization of denoising trajectory.
                x_rev = float(lam) * x_t_start + (1.0 - float(lam)) * x_rec
                ax_r.plot(
                    t_sec,
                    x_rev,
                    color=c,
                    lw=1.0,
                    alpha=0.9,
                    label=f"reverse step {int((1.0 - lam) * 100)}%",
                )
    ...
    if bool(args.generate_ddpm_figures):
        _log_section("Figure-only mode: generate DDPM paper figures (no training/evaluation)")
