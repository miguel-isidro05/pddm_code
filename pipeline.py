...
def main() -> None:
    ...
    if not args.skip_sgm:
        ...
        for spec in EXPERIMENTS:
            ...
            ckpt_path = ckpt_dir / f"sgm_{sde_name}_{args.sgm_backbone}_{spec.target}_steps{int(args.sgm_train_steps)}.pt"
            ...
            if bool(args.sgm_cache) and (not loaded_from_ckpt):
                try:
                    ...
                    print(f"[SGM ckpt] saved: {ckpt_path.name}")
                except Exception as e:
                    print(f"[SGM ckpt] save failed ({ckpt_path.name}): {e}")
