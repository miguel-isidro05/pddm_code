...
def run_csp_lda(
    *,
    ...
    # Classic CSP setup (literature-style): no shrinkage, per-epoch covariance.
    CSP_CFG_PRIMARY = dict(n_components=4, reg=None, log=True, norm_trace=True, cov_est="epoch")
    CSP_CFG_FALLBACK = dict(n_components=4, reg=None, log=True, norm_trace=True, cov_est="epoch")
    ...
    try:
        ...
        print(f"[WARN] CSP primary config failed ({e}); retrying classic CSP config.")
    ...
    if not np.isfinite(F_train).all() or not np.isfinite(F_test_real).all() or not np.isfinite(F_test_hybrid).all():
        try:
            print("[WARN] Non-finite CSP features detected; retrying classic CSP config.")
            F_train, F_test_real, F_test_hybrid = _fit(CSP_CFG_FALLBACK)
        except Exception:
    ...
    if (np.std(F_train, axis=0) < 1e-8).any() or np.linalg.matrix_rank(F_train) < min(F_train.shape):
        try:
            print("[WARN] Degenerate CSP train features; retrying classic CSP config.")
            F_train, F_test_real, F_test_hybrid = _fit(CSP_CFG_FALLBACK)
        except Exception:
            pass
...
def run_fbcsp_lda(
    *,
    ...
    # Classic FBCSP config: no shrinkage + per-epoch covariance.
