...
def main() -> None:
    ...
    def _generate_ddpm_paper_figures() -> None:
        ...
        # ------------------------------
        # Figure 2: forward and reverse iterations, saved per channel.
        # ------------------------------
        fig2_dir = out_dir / "figure2_forward_reverse"
        fig2_dir.mkdir(parents=True, exist_ok=True)
        sample_idx = 0
        t_grid_forward = [0, 50, 150, 350, 700, 999]
        colors_fwd = plt.cm.viridis(np.linspace(0.1, 0.95, len(t_grid_forward)))
        ...
        for ch_name, ch_idx in zip(wanted, ch_idxs):
            ...
            # Forward plot with several timesteps.
            fig_f, ax_f = plt.subplots(1, 1, figsize=(10, 3.5))
            ax_f.plot(t_sec, x0, color="black", lw=1.2, alpha=0.9, label="x0 (original)")
            for c, tt in zip(colors_fwd, t_grid_forward):
                ab = float(alphas_cumprod_fig[int(tt)])
                xt = np.sqrt(max(ab, 1e-12)) * x0 + np.sqrt(max(1.0 - ab, 1e-12)) * eps
                ax_f.plot(t_sec, xt, color=c, lw=1.0, alpha=0.85, label=f"forward t={tt}")
            ax_f.set_title(f"{ch_name} — Forward diffusion iterations")
            ax_f.set_xlabel("Time (s)")
            ax_f.set_ylabel("Amplitude (a.u.)")
            ax_f.grid(alpha=0.25, lw=0.4)
            ax_f.legend(loc="upper right", fontsize=7, ncol=2, frameon=False)
            fig_f.tight_layout()
            fig_f.savefig(fig2_dir / f"figure2_{ch_name.lower()}_forward_iterations.png", dpi=300, bbox_inches="tight")
            plt.close(fig_f)
            ...
            cond_t = torch.from_numpy(cond_np).to(device)
            reverse_steps = [999, 700, 500, 350, 150, 50, 0]
            reverse_series = _sample_with_trajectory(ddpm_models_fig[spec.target], cond_t, trajectory_steps=reverse_steps)
            colors_rev = plt.cm.plasma(np.linspace(0.15, 0.95, len(reverse_steps)))
            fig_r, ax_r = plt.subplots(1, 1, figsize=(10, 3.5))
            ax_r.plot(t_sec, x0, color="black", lw=1.2, alpha=0.9, label="x0 (original)")
            for c, tt in zip(colors_rev, reverse_steps):
                ...
                ax_r.plot(
                    ...
                    lw=1.0,
                    alpha=0.9,
                    label=f"reverse t={int(tt)}",
                )
            ax_r.set_title(f"{ch_name} — Reverse process iterations")
            ax_r.set_xlabel("Time (s)")
            ax_r.set_ylabel("Amplitude (a.u.)")
            ax_r.grid(alpha=0.25, lw=0.4)
            ax_r.legend(loc="upper right", fontsize=7, ncol=2, frameon=False)
            fig_r.tight_layout()
            fig_r.savefig(fig2_dir / f"figure2_{ch_name.lower()}_reverse_iterations.png", dpi=300, bbox_inches="tight")
            plt.close(fig_r)
        ...
        ep_real_2ch = mne.EpochsArray(X_test_real[:, ch_idxs, :], info_2ch, verbose=False)
        ep_hyb_2ch = mne.EpochsArray(x_hybrid[:, ch_idxs, :], info_2ch, verbose=False)
        ...
        # Wavelet TFR (average over epochs) on 8-30 Hz.
        freqs_tfr = np.arange(8.0, 31.0, 1.0, dtype=np.float64)
        n_cycles = freqs_tfr / 2.0
        x_real_arr = np.asarray(X_test_real[:, ch_idxs, :], dtype=np.float32)
        x_hyb_arr = np.asarray(x_hybrid[:, ch_idxs, :], dtype=np.float32)
        pwr_real = tfr_array_morlet(
            x_real_arr,
            sfreq=sf,
            freqs=freqs_tfr,
            n_cycles=n_cycles,
        ...
        pwr_real_m = np.mean(pwr_real, axis=0)
        pwr_hyb_m = np.mean(pwr_hyb, axis=0)
        ...
        for row_i, ch_name in enumerate(wanted):
            ...
            # Wavelet heatmap: one image per condition.
            for cond_name, pwr in [
                ("original", pwr_real_m[row_i]),
                ("hybrid", pwr_hyb_m[row_i]),
            ]:
                ...
                im = ax_w.imshow(
                    ...
                    aspect="auto",
                    origin="lower",
                    extent=[t_sec[0], t_sec[-1], float(freqs_tfr[0]), float(freqs_tfr[-1])],
                    cmap="viridis",
                )
