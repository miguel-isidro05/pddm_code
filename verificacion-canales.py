"""
Verificación de canales (BCI-IV 2a): DDPM reconstruction quality per channel.

Qué hace:
- Carga AxxT.gdf (TRAIN) y AxxE.gdf (EVAL)
- Preprocesa similar a pipeline2.py (bandpass 8–30 + notch 50; ICA fold-safe a nivel epochs)
- Epoching 0–4s post-cue
- Ajusta z-score con stats de TRAIN (anti-leakage)
- Para cada canal EEG (excepto C3/C4/Cz):
  - elige 2 inputs EEG más cercanos (por posiciones del montage)
  - entrena un DDPM condicional (inputs + x_t -> pred noise)
  - evalúa en EVAL: MSE y Pearson entre reconstrucción y canal real

Salida:
- Tabla por canal con inputs, MSE, Pearson (ordenada por Pearson asc/desc configurable)
- (Opcional) guarda CSV
"""
...
def _per_channel_logvar_mi_note() -> str:
    return (
        "Nota: este script NO selecciona targets por MI; calcula MSE/Pearson para todos los canales (excepto C3/C4/Cz)."
    )


def _choose_two_nearest_inputs(
    *,
    target: str,
    ...
    if target not in ch_pos:
        # fallback: nearest by index order
        idx = eeg_ch_names.index(target)
        cand = [c for c in eeg_ch_names if c != target and c not in exclude]
        if len(cand) < 2:
            raise ValueError("Not enough channels to pick inputs.")
        # pick neighbors around index
        left = [c for c in cand if eeg_ch_names.index(c) < idx]
        right = [c for c in cand if eeg_ch_names.index(c) > idx]
        pick = []
        if left:
            pick.append(left[-1])
        if right:
            pick.append(right[0])
        while len(pick) < 2:
            pick.append(cand[len(pick)])
        return pick[0], pick[1]

    tgt_pos = np.asarray(ch_pos[target], dtype=float)
    candidates = [c for c in eeg_ch_names if c != target and c not in exclude and c in ch_pos]
    if len(candidates) < 2:
        raise ValueError(f"Not enough positioned channels to pick inputs for target={target}")
    candidates_sorted = sorted(
        candidates,
        key=lambda c: float(np.linalg.norm(np.asarray(ch_pos[c], dtype=float) - tgt_pos)),
    )
...
def main() -> None:
    ...
    print(_per_channel_logvar_mi_note())
    print(f"[CFG] subject={subject_id} | ddpm_epochs={int(args.ddpm_epochs)} | exclude_targets={sorted(exclude_targets)}")
    ...
    # Montage positions for nearest-neighbor inputs
    pos = {}
    try:
        montage = epochs_train.get_montage()
        if montage is not None:
            pos = montage.get_positions().get("ch_pos", {}) or {}
    except Exception:
        pos = {}
    ch_pos = {k: np.asarray(v, dtype=float) for k, v in pos.items() if k in set(eeg_ch_names)}
    ...
    for target in eeg_ch_names:
        if target in exclude_targets:
            continue
        inp1, inp2 = _choose_two_nearest_inputs(
            target=target, eeg_ch_names=eeg_ch_names, ch_pos=ch_pos, exclude=exclude_targets
        )
