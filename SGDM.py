    ...
    *,
    ...
    for epoch in range(epochs):
        ...
        for _, (batch_inputs, batch_targets) in tqdm(
            ...
            if float(score_clip) > 0:
                b = int(score.shape[0])
                norms = torch.norm(score.reshape(b, -1), dim=1)
                scales = (float(score_clip) / (norms + 1e-12)).clamp(max=1.0).view(b, 1, 1)
                score = score * scales
...
def sample_from_model(
    ...
    *,
    ...
    beta_min: float = 0.1,
    beta_max: float = 20.0,
    # New (Song+Pedrotti): sampler choices
    sampler: str = "pc",  # pc | ode
    ode_method: str = "rk4",  # euler | rk4
    init_corrector_steps: int = 0,
    score_clip: float = 0.0,
):
    """
    Predictor–Corrector sampling for VP-SDE (default) or VE-SDE, or probability flow ODE.
    ...
    with torch.no_grad():
        ...
        ode_method = str(ode_method).lower().strip()
        if sampler not in {"pc", "ode"}:
            raise ValueError(f"sampler must be pc|ode. Got {sampler}")
        if ode_method not in {"euler", "rk4"}:
            raise ValueError(f"ode_method must be euler|rk4. Got {ode_method}")

        def _clip(score: torch.Tensor) -> torch.Tensor:
            m = float(score_clip)
            if m <= 0:
                return score
            b = int(score.shape[0])
            n = torch.norm(score.reshape(b, -1), dim=1)
            s = (m / (n + 1e-12)).clamp(max=1.0).view(b, 1, 1)
            return score * s
        ...
        if int(init_corrector_steps) > 0:
            ...
            if sde_type == "vp":
                g0 = vp_diffusion(t0, beta_min=beta_min, beta_max=beta_max).view(batch_size, 1, 1)
            else:
                g0 = diffusion_coeff(t0, sigma).view(batch_size, 1, 1)
            noise_norm0 = float(np.sqrt(np.prod(x.shape[1:])))
            for _ in range(int(init_corrector_steps)):
                model_input = torch.cat([conditioning_inputs, x], dim=1)
                score0 = _clip(model_s(model_input, t0))
                grad_norm0 = torch.norm(score0.reshape(batch_size, -1), dim=1).mean().item()
                step0 = 2.0 * (float(snr) * noise_norm0 / (float(grad_norm0) + 1e-12)) ** 2
                step0 = float(min(step0, 0.01))
                x = x + step0 * score0 + torch.sqrt(torch.tensor(2.0 * step0, device=device)) * torch.randn_like(x)

        if sampler == "ode":
            # Probability flow ODE: dx = [f(x,t) - 0.5 g(t)^2 score] dt
            def f_ode(x_in: torch.Tensor, t_in: torch.Tensor) -> torch.Tensor:
                model_input = torch.cat([conditioning_inputs, x_in], dim=1)
                score = _clip(model_s(model_input, t_in))
                if sde_type == "vp":
                    beta_t = vp_beta(t_in, beta_min=beta_min, beta_max=beta_max).view(batch_size, 1, 1)
                    drift = -0.5 * beta_t * x_in
                    return drift - 0.5 * beta_t * score
                g = diffusion_coeff(t_in, sigma).view(batch_size, 1, 1)
            ...
            for i in range(int(t_seq.numel()) - 1):
                ...
                dti = float(t1[0].item() - t0[0].item())  # negative
                if ode_method == "euler":
                    x = x + f_ode(x, t0) * dti
                else:
                    tm = (t0 + t1) * 0.5
                    k1 = f_ode(x, t0)
                    k2 = f_ode(x + 0.5 * dti * k1, tm)
                    k3 = f_ode(x + 0.5 * dti * k2, tm)
                    k4 = f_ode(x + dti * k3, t1)
                    x = x + (dti / 6.0) * (k1 + 2.0 * k2 + 2.0 * k3 + k4)
            return x
        ...
        for idx, t_scalar in enumerate(t_seq):
            ...
            for _ in range(int(n_steps_each)):
                ...
                model_input = torch.cat([conditioning_inputs, x], dim=1)
                score = _clip(model_s(model_input, t))
