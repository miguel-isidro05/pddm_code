...
def run_csp_lda(
    *,
    ...
    return {
        "no_cv": {"acc_real": acc_real, "acc_hybrid": acc_h},
        "cv": {
...
def run_unet_classifier(
    *,
    ...
    return {
        "no_cv": {"acc_real": float(metrics_real["acc"]), "acc_hybrid": float(metrics_hybrid["acc"])},
        "cv": {
            "acc_real": float(cv_real["cv_acc"]),
            "acc_real_std": float(cv_real["cv_acc_std"]),
            "acc_hybrid": float(cv_h["cv_acc"]),
...
def run_eegnet(
    *,
    ...
    Xtr = np.asarray(X_train, dtype=np.float32)[:, None, :, :]
    Xte_r = np.asarray(X_test_real, dtype=np.float32)[:, None, :, :]
    Xte_h = np.asarray(X_test_hybrid, dtype=np.float32)[:, None, :, :]
    ytr = np.asarray(y_train_enc, dtype=np.int64)
    yte = np.asarray(y_test_enc, dtype=np.int64)
...
def main() -> None:
    ...
    # Solo Motor Imagery: left (2) y right (3). Excluye "word" (7).
    allowed_labels = {2, 3}
    ...
    if not args.skip_sgm:
        ...
        # Carga SGDM.py desde la misma carpeta del script (evita depender de PYTHONPATH/cwd).
        try:
            ...
            if not sgdm_path.exists():
                raise FileNotFoundError(
                    f"No se encontró `SGDM.py` en {sgdm_path.parent}. "
                    "Copia `SGDM.py` junto a `pipeline.py` o ejecuta con `--skip-sgm`."
                )

            spec = importlib.util.spec_from_file_location("SGDM", str(sgdm_path))
            if spec is None or spec.loader is None:
                raise ImportError(f"No pude crear spec de import para {sgdm_path}")
            sgm_mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(sgm_mod)
        except Exception as e:
            raise ModuleNotFoundError(
                "No se pudo importar ScoreNet/SGM. "
                "Asegúrate de tener `SGDM.py` en la misma carpeta que `pipeline.py` "
                "o corre con `--skip-sgm`."
            ) from e
