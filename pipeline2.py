...
def _default_experiments_2a() -> list[ExperimentSpec2A]:
    ...
    Keep the original idea but REMOVE the two targets that caused excessive covariance drift:
      - remove: FC3, C6
    Replace them with more stable / redundant channels for CSP/FBCSP:
      - CP3, CP4
    """
    return [
        ExperimentSpec2A("C1", ("C3", "Cz")),
        ExperimentSpec2A("C2", ("C4", "Cz")),
        ExperimentSpec2A("CP3", ("C3", "CP1")),
        ExperimentSpec2A("POz", ("Pz", "CPz")),
        ExperimentSpec2A("CP4", ("C4", "CP2")),
...
def run_eegnet_with_kappa(
    *,
    ...
    model = p.EEGNetModel(chans=int(X_train.shape[1]), classes=n_classes, time_points=int(X_train.shape[2])).to(device)
