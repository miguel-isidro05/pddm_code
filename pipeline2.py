...
def run_csp_lda_robust(
    *,
    ...
    """CSP+LDA with covariance regularization to avoid NaNs."""
    ...
    def _fit_transform_with_cfg(cfg: dict) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        csp = CSP(**cfg)
        F_tr = csp.fit_transform(np.asarray(X_train, np.float64), y_train_enc)
        F_r = csp.transform(np.asarray(X_test_real, np.float64))
        F_h = csp.transform(np.asarray(X_test_hybrid, np.float64))
        return F_tr, F_r, F_h
