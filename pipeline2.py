...
def _print_accuracy_table(title: str, results: dict[str, dict[str, object]]) -> None:
    """
    Local summary table (avoid depending on pipeline.py helper being present on server).
    Expected `results[k]` is the dict returned by run_* functions in pipeline.py/pipeline2.py:
    - has `no_cv` and optionally `cv`.
    """
    print("\n" + "=" * 80)
    print(title)
    print("=" * 80)
    ...
    try:
        ...
        for r in rows:
            print({k: _fmt(v) for k, v in r.items()})
...
def _default_experiments_2a() -> list[ExperimentSpec2A]:
    """
    Default TARGET channels for BCI-IV 2a with two adjacent inputs.
    Chosen to avoid synthesizing the most MI-critical electrodes (C3/C4/Cz),
    and to favor high Pearson by synthesizing spatially redundant neighbors.
    """
    return [
        # IMPORTANT: do NOT synthesize C3/C4/Cz (motor-critical). Use them as inputs only.
        # 6 "less critical / more reconstructible" targets:
...
def _auto_experiments_2a(
    *,
    ...
    targets_set = set(str(t) for t in targets)
    specs: list[ExperimentSpec2A] = []
    for tgt in targets:
        tgt_pos = np.asarray(pos[tgt], dtype=float)
        # Inputs: 2 nearest *real* channels by spatial distance.
        # Important: do NOT condition on channels that are also being synthesized (avoids compounding errors).
        input_pool = [c for c in ch_with_pos if c != tgt and c not in targets_set]
        if len(input_pool) < 2:
            input_pool = [c for c in ch_with_pos if c != tgt]
        neighbors = sorted(
            input_pool,
...
def main() -> None:
    ...
    parser.add_argument("--n-targets", type=int, default=6, help="Number of channels to synthesize (only when --auto-specs).")
