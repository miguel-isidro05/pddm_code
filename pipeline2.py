...
def _preprocess_like_bci3(
    ...
    *,
    ...
    - then: bandpass 8–30 + notch(50) + annotate_muscle_zscore
    ...
    # Drop EOG channels (we do NOT run ICA in this pipeline variant).
...
def main() -> None:
    ...
    PREPROC_ID = "2a|bp8-30|notch50|no_ica|drop_eog|zscore_train|t0-4s|no_per_epoch_z|farthest_targets|v4"
    ...
    # This pipeline variant DOES NOT use ICA (to avoid leakage/instability).
    ...
    #
    # Here we do the FULL-TRAIN z-score fit for the held-out EVAL set. Later, CV will redo z-score per fold.
    # ------------------------------------------------------------------------------
    info_all = epochs_train.info.copy()
    # Keep raw (pre-z) epochs for TRAIN-only CV
    ...
    if bool(args.run_train_cv) and (not bool(args.skip_ddpm)):
        ...
        # We run CV on the raw (pre-zscore) TRAIN epochs,
        # then within each fold:
        # 1) fit z-score on fold-train, apply to train+val
        ...
        for fold_i, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(len(y_train_cv)), y_train_cv), start=1):
            ...
            # z-score fold-safe (fit on train only)
            Xtr_z, Xva_z = _zscore_fit_apply(Xtr_all, Xva_all)
