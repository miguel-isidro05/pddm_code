...
def _load_eval_labels_mat(*, labels_dir: Path, subject_id: str) -> np.ndarray:
    ...
    if not mat_path.exists():
        raise FileNotFoundError(
            f"No encontré {mat_path}. Descarga/extráe true_labels.zip en {labels_dir}/."
        )
...
def main() -> None:
    ...
    parser.add_argument("--labels-dir", type=str, default="second_session_labels", help="Folder containing AxxE.mat (classlabel)")
    ...
    epochs_eval = mne.Epochs(
        raw_eval,
        events_eval,
        event_id={"cue": int(event_id_eval[eval_cue_key])},
        tmin=0.0,
        tmax=4.0,
        reject=None,
        baseline=None,
    ...
    y_eval = _load_eval_labels_mat(labels_dir=labels_dir, subject_id=subject_id)

    if len(X_eval) != len(y_eval):
        min_n = min(len(X_eval), len(y_eval))
        print(f"[WARN] EVAL mismatch: epochs={len(X_eval)} labels={len(y_eval)}. Aligning min_n={min_n}")
        X_eval = X_eval[:min_n]
        y_eval = y_eval[:min_n]
    ...
    if not args.skip_ddpm:
        ...
        # Hybrid (DDPM)
        X_test_hybrid_ddpm = p.build_hybrid_epochs_ddpm(
            X_eval,
            specs=[p.ExperimentSpec(target=s.target, inputs=s.inputs, mse_reported=0.0, pearson_reported=0.0) for s in specs],
