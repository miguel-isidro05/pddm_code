...
def run_csp_lda_robust(
    *,
    ...
    cv_real = _fold_stats_from_fixed_preds(y_true=y_test_enc, y_pred=pred_real, n_splits=5)
    cv_h = _fold_stats_from_fixed_preds(y_true=y_test_enc, y_pred=pred_h, n_splits=5)
    return {
...
def run_fbcsp_lda_safe(
    *,
    ...
    """
    Use pipeline.py FBCSP first; if it fails due to NaNs/Infs inside CSP, retry with regularized CSP.
    """
    try:
    ...
    # Notebook uses cov_est="epoch". We try that first; if it still produces non-finite values,
    # we retry with cov_est="concat" (more stable, sometimes slightly worse).
    CSP_CFG_PRIMARY = dict(n_components=per_band, reg="ledoit_wolf", log=True, norm_trace=True, cov_est="epoch")
    CSP_CFG_FALLBACK = dict(n_components=per_band, reg="ledoit_wolf", log=True, norm_trace=True, cov_est="concat")
    LDA_CFG = dict(solver="lsqr", shrinkage="auto")
    ...
    def _fit_one_band(cfg: dict, Xtr_b: np.ndarray, Xte_r_b: np.ndarray, Xte_h_b: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        csp_b = CSP(**cfg)
        Ftr = csp_b.fit_transform(Xtr_b, y_train_enc)
        Fr = csp_b.transform(Xte_r_b)
        Fh = csp_b.transform(Xte_h_b)
        return Ftr, Fr, Fh

    # First attempt: cov_est="epoch"
    for (l_f, h_f) in bands:
        ...
        try:
            Ftr, Fr, Fh = _fit_one_band(CSP_CFG_PRIMARY, Xtr_b, Xte_r_b, Xte_h_b)
        except Exception:
            Ftr, Fr, Fh = _fit_one_band(CSP_CFG_FALLBACK, Xtr_b, Xte_r_b, Xte_h_b)
        F_train_list.append(Ftr)
        F_test_real_list.append(Fr)
        F_test_hybrid_list.append(Fh)
...
def run_eegnet_with_kappa(
    *,
    ...
    # "CV" as fold-stats on fixed train→test predictions (no retraining on TEST)
...
def main() -> None:
    ...
    parser.add_argument("--eegnet-lr", type=float, default=3e-4)
