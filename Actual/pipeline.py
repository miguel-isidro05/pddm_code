...
def build_hybrid_epochs_ddpm(
    ...
    *,
    ...
    """Reemplaza canales target por reconstrucción DDPM (con per-epoch z-score + clipping)."""
    ...
    for spec in specs:
        ...
        for s in range(0, n, gen_batch_size):
            ...
            syn_np = np.clip(syn_np, -float(clip_val), float(clip_val)).astype(np.float32, copy=False)
    ...
    *,
    ...
    for spec in specs:
        ...
        for s0 in range(0, int(replace_idx.size), int(gen_batch_size)):
            ...
            batch = epochs_h[idx_batch]  # (B, C, T)
            cond = np.stack([batch[:, input_idxs[0], :], batch[:, input_idxs[1], :]], axis=1)  # (B, 2, T)
            cond_t = torch.from_numpy(cond).to(device)
            with torch.no_grad():
                syn = sample_from_model(
...
def main() -> None:
    ...
    if bool(args.use_official_test):
        ...
        # preprocess test with SAME filter + ICA fitted on train
        ...
        raw_test = ica.apply(raw_test.copy(), verbose=False)
    ...
    if bool(args.only_eegnet):
        ...
        for fold_i, (tr_idx, va_idx) in enumerate(splits, start=1):
            ...
            X_tr, X_va = _zscore_fit_apply(X_tr_raw, X_va_raw)
        ...
        print(
            {
                ...
                "CV Acc Original (mean/std)": f"{cv_acc_m:.3f} ({cv_acc_s:.3f})",
